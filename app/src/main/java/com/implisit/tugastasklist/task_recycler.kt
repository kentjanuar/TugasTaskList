...
class task_recycler(private val listTask: ArrayList<task>) : RecyclerView.Adapter<task_recycler.ListViewHolder>() {
    ...
    inner class ListViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        var _imageTask: ImageView = itemView.findViewById(R.id.imageTask)
    ...
    override fun onBindViewHolder(holder: ListViewHolder, position: Int) {
        ...
        if (resourceId != 0) {
            holder._imageTask.setImageResource(resourceId)
        } else {
            holder._imageTask.setImageResource(R.drawable.error)
        }
